...
int listSize=0;
struct Eleman* head; // Başlangıç düğümü için referans
...
void listeBasinaEkle(int deger) {
	...
	if(head == NULL) { 	// Hiç eleman yoksa
		head = yeniEleman; //  yeni elemanı baş eleman yap
		printf("%d ilk eleman olarak eklendi\n",yeniEleman->data);
		return;
	}
	
	head->onceki = yeniEleman;	// Listenin baş elemanından önceki eleman olarak yeni elemanı göster
	yeniEleman->sonraki = head; // Yeni elemandan sonraki eleman olarak baş elemanı göster
	head = yeniEleman;// yeni elemanı baş eleman yap
...
void listeBasindanSil(){ 
	...
	struct Eleman* geciciEleman=head; // Listede dolaşmak için yeni bir eleman oluştur
	...
	head=geciciEleman->sonraki;				// Listenin ilk elemanı olarak en baştaki elemandan bir sonraki elemanı seç
	head->onceki=NULL;						// Listenin en başındaki elemandan önce bir eleman olamaz
...
void listeArasinaEkle(){
	...
	struct Eleman* geciciEleman=head; // Listede dolaşmak için geçici eleman
	...
	if(index==1){
		...
		for(int i=2;i<index;i++)		// Eklenecek elemanın indexine kadar git
...
void listeArasindanSil(){
	...
	struct Eleman* geciciEleman=head; // Listeyi dolaşmak için geçici eleman oluştur
	...
	if(index==1){
		...
		for(int i=1;i<index;i++)		// Silinecek elemanın indexine kadar listede ilerle
			geciciEleman=geciciEleman->sonraki;
...
void yazdir() {
	...
	struct Eleman* geciciEleman = head; // Listeyi dolaşmak için geçici eleman 
...
int main() {
	int a;
	head = NULL; // Boş liste için ilk referans.,
